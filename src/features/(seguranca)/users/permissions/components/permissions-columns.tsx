...
function ActionsColumn({ row }: { row: IPermissions }) {
  const { onOpen } = useModalInstance<IPermissions>(
    MODAL_KEYS.CREATE_PERMISSION,
  );
  const { onOpen: onConfirmDelete } =
...
export const permissionsColumns = (): ColumnDef<IPermissions>[] => {
  return [
    {
      accessorKey: "name",
      header: ({ column }) => (
