...
function ActionsColumn({ row }: { row: IRoles }) {
  const { onOpen } = useModalInstance<IRoles>(MODAL_KEYS.CREATE_ROLE);
  const { onOpen: onConfirmDelete } = useModalInstance(MODAL_KEYS.DELETE_ROLE);
...
export const rolesColumns = (): ColumnDef<IRoles>[] => {
  return [
    {
      accessorKey: "name",
      header: ({ column }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const grupos = row.original.permissoes_grupos || [];
        const total = grupos.reduce(
          (acc, grupo) => acc + (grupo.permissoes?.length || 0),
          0,
        );
        return (
          <div className="flex flex-wrap gap-1">
            <Badge variant="secondary">{grupos.length} grupo(s)</Badge>
            <Badge variant="outline">{total} permissão(ões)</Badge>
          </div>
        );
      },
    },
