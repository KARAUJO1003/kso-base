...
} from "@/features/(cadastros)/depositos/utils/constants";
import { Can } from "@/lib/auth/components/can";
import { Feature } from "@/lib/feature-flags/components/feature-flag";
import { MODULE_ROUTE, MODULE_CONFIG, PERMISSIONS } from "@/features/(cadastros)/depositos/utils/module-utils";

export const ExcluirDeposito = ({ row }: { row: Row<IDeposito> }) => {
  const onDeleteMutation = useDelete({
    mutationKey: [MUTATION_KEYS_DEPOSITO.DELETE],
    route: MODULE_ROUTE,
