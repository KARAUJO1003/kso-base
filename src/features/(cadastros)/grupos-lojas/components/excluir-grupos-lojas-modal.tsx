...
} from "@/features/(cadastros)/grupos-lojas/utils/constants";
import { Can } from "@/lib/auth/components/can";
import { Feature } from "@/lib/feature-flags/components/feature-flag";
import { MODULE_ROUTE, MODULE_CONFIG, PERMISSIONS } from "@/features/(cadastros)/grupos-lojas/utils/module-utils";

export const ExcluirGrupoLoja = ({ row }: { row: Row<IGrupoLoja> }) => {
  const onDeleteMutation = useDelete({
    mutationKey: [MUTATION_KEYS_GRUPOLOJA.DELETE],
    route: MODULE_ROUTE,
