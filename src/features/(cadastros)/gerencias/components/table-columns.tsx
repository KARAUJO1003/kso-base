...
import { EditarGerencia } from "@/features/(cadastros)/gerencias/components/editar-gerencia-modal";
import { ExcluirGerencia } from "@/features/(cadastros)/gerencias/components/excluir-gerencia-modal";

const codigoColumn = (): ColumnDef<IGerencia, string> => {
  return {
    accessorKey: "codigo",
    id: "codigo",
    header: ({ column }) => (
...
const nomeColumn = (): ColumnDef<IGerencia, string> => {
  return {
    accessorKey: "nome",
    id: "nome",
    header: ({ column }) => (
...
const createdAtColumn = (): ColumnDef<IGerencia, string> => {
  return {
    accessorFn: (row) => {
...
const updatedAtColumn = (): ColumnDef<IGerencia, string> => {
  return {
    accessorFn: (row) => {
...
const actionsColumn = (): ColumnDef<IGerencia, any> => {
  return {
    accessorKey: "actions",
    id: "actions",
    header: ({ column }) => (
    ...
    cell: (info) => {
      ...
      return (
        <div className="flex items-center gap-1">
          <EditarGerencia row={info.row} />
          <ExcluirGerencia row={info.row} />
        </div>
      );
    },
  };
...
export const columns = (): ColumnDef<IGerencia, any>[] => [
  codigoColumn(),
  nomeColumn(),
  createdAtColumn(),
  updatedAtColumn(),
