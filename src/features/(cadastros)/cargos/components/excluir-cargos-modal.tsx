...
import {
  QUERIES_KEYS_CARGO,
  MUTATION_KEYS_CARGO,
} from "@/features/(cadastros)/cargos/utils/constants";
import { Can } from "@/lib/auth/components/can";
import { Feature } from "@/lib/feature-flags/components/feature-flag";
import { MODULE_ROUTE, MODULE_CONFIG, PERMISSIONS } from "@/features/(cadastros)/cargos/utils/module-utils";

export const ExcluirCargo = ({ row }: { row: Row<ICargo> }) => {
  const onDeleteMutation = useDelete({
    mutationKey: [MUTATION_KEYS_CARGO.DELETE],
    route: MODULE_ROUTE,
