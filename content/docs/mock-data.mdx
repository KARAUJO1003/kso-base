...
Troca o **adapter do axios** (`src/lib/axios-instance.ts`) por um adapter
fake (`src/lib/mock/adapter.ts`), sem tocar em nenhum hook ou componente
acima dessa camada — `useFetch`/`useCreate`/`useUpdate`/`useDelete`
continuam chamando `api.get`/`api.post`/... normalmente, sem saber que a
resposta não veio de rede.

```ts
if (DEMO_CONFIG.mockApi) {
  axiosInstance.defaults.adapter = async (config) => {
    const { mockAdapter } = await import("@/lib/mock/adapter"); // code-split
...
1. Apague a pasta `src/lib/mock/`.
2. Remova o bloco `if (DEMO_CONFIG.mockApi) { ... }` de
...
Como cada mudança está isolada atrás de uma checagem de flag no início da
função (nunca no meio da lógica de negócio), remover é reverter um `if` por
arquivo — não precisa desfazer nada no restante do código.
